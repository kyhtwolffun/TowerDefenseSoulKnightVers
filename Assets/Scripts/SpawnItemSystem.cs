...
public class SpawnItemSystem : MonoBehaviour
{
    ...
    [SerializeField] private List<CollectableDataBase> collectableDataList;
    ...
    private void Start()
    {
        for (int i = 0; i < collectableDataList.Count; i++)
        {
            SpawnItem(collectableDataList[i], (Vector2)transform.position + Vector2.right * Random.Range(-5f,5f) + Vector2.up * Random.Range(-5f, 5f));
        }
    }
